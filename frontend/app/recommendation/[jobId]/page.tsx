      ...
      {isLoading ? (
        ...
        <>
          ...
          <motion.div
            ...
            className="relative card p-6 md:p-8 mb-8 overflow-hidden bg-surface-2 border border-border"
          >
            {/* Background glow */}
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle, rgba(18,179,163,0.12) 0%, transparent 70%)",
              }}
            />

            <div className="relative z-10 flex flex-col sm:flex-row items-start gap-6">
              {/* Trophy icon */}
              <div className="w-14 h-14 rounded-xl bg-warning-500/10 border border-warning-500/25 flex items-center justify-center shrink-0">
                <Trophy className="w-7 h-7 text-warning-400" />
              </div>

              <div className="flex-1 min-w-0">
                <p className="text-xs text-warning-400 uppercase tracking-widest font-semibold mb-1">
                  Recommended Model
                </p>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {rec.recommended_pipeline.map((step) => (
                    <span
                      key={step}
                      className="text-xs px-2.5 py-1 rounded-md bg-surface-3 border border-border-subtle text-text-secondary font-mono"
                    >
                      {step}
                    </span>
                  ))}
          ...
          >
            <GlassCard>
              <p className="text-xs text-text-muted uppercase tracking-wider font-semibold mb-3 flex items-center gap-1.5">
              ...
              <GlassCard>
                ...
                <ul className="space-y-2">
                  {rec.implementation_tips.map((tip, i) => (
                    <li key={i} className="flex items-start gap-2.5">
                      <span className="text-xs text-brand-400 font-bold font-mono shrink-0 w-4">
                        {i + 1}.
                      </span>
